...
export class GameController {
    ...
    try {
      ...
      if (error instanceof Error) {
        const errors: Record<string, [number, string]> = {
          ...
          GAME_SESSION_QUESTION_NOT_FOUND: [
            404,
            'game_session_question_not_found',
          ],
          GAME_SESSION_QUESTION_NOT_PENDING: [
            409,
            'game_session_question_not_pending',
          ],
    ...
    try {
      ...
      if (error instanceof Error) {
        const errors: Record<string, [number, string]> = {
          ...
          GAME_JOKER_INSUFFICIENT_OPTIONS: [
            409,
            'game_joker_insufficient_options',
          ],
        };
  ...
  private gameError(
    ...
    if (error instanceof Error) {
      const map: Record<string, [number, string]> = {
        ...
        GAME_SESSION_ALREADY_ACTIVE: [409, 'game_session_already_active'],
        GAME_SESSION_NOT_FINISHABLE: [409, 'game_session_not_finishable'],
        GAME_SESSION_NO_NEXT_QUESTION: [409, 'game_session_no_next_question'],
        GAME_ANSWER_INVALID: [400, 'game_answer_invalid'],
      };
      const m = map[error.message];
      if (m) return reply.code(m[0]).send({ message: request.t(m[1]) });
    }
    throw error;
  }
  start = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      return reply
        .code(201)
        .send(await this.gameplay.start(request.authenticatedUser!.id));
    } catch (e) {
  ...
  answer = async (
    ...
    try {
      return await this.gameplay.answer({
        userId: request.authenticatedUser!.id,
        sessionId: request.params.sessionId,
        sessionQuestionId: request.body.session_question_id,
        answerOptionId: request.body.answer_option_id,
      });
    } catch (e) {
      return this.gameError(e, request, reply);
    }
  ...
  finish = async (
    ...
    try {
      return await this.gameplay.finish({
        userId: request.authenticatedUser!.id,
        sessionId: request.params.sessionId,
      });
    } catch (e) {
  ...
  ranking = async (
    ...
    try {
      return await this.gameplay.ranking({
        userId: request.authenticatedUser!.id,
  ...
  internationalRanking = (r: FastifyRequest<any>, p: FastifyReply) =>
    this.ranking(r, p, false);
  nationalRanking = (r: FastifyRequest<any>, p: FastifyReply) =>
    this.ranking(r, p, true);
  myRanking = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      return await this.gameplay.myRanking(request.authenticatedUser!.id);
    } catch (e) {
      return this.gameError(e, request, reply);
    }
  };
