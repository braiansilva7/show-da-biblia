...
type CategoryBody = { name?: string; description?: string | null; active?: boolean };
...
export class CategoryController {
  constructor(
    @inject(CategoryListerUseCase) private readonly lister: CategoryListerUseCase,
    @inject(CategoryCreatorUseCase) private readonly creator: CategoryCreatorUseCase,
    @inject(CategoryUpdaterUseCase) private readonly updater: CategoryUpdaterUseCase,
    @inject(CategoryDeleterUseCase) private readonly deleter: CategoryDeleterUseCase
  ) {}
  ...
  list = async (request: FastifyRequest) => {
    ...
    return { ...(await this.lister.execute({ page, limit, search: query.search })), page, limit };
  };

  create = async (request: FastifyRequest<{ Body: CategoryBody }>, reply: FastifyReply) => {
    const input = normalizeInput(request.body);
    if (!input || input.name === undefined)
      return reply.code(400).send({ message: request.t('category_create_invalid_input') });
    try {
      ...
      if (error instanceof Error && error.message === 'CATEGORY_NAME_ALREADY_EXISTS')
        return reply.code(409).send({ message: request.t('category_name_already_exists') });
  ...
  update = async (request: FastifyRequest<{ Params: { id: string }; Body: CategoryBody }>, reply: FastifyReply) => {
    const input = normalizeInput(request.body);
    if (!input || !Object.keys(input).length)
      return reply.code(400).send({ message: request.t('category_update_invalid_input') });
    try {
      ...
      if (error instanceof Error && error.message === 'CATEGORY_HAS_QUESTIONS')
        return reply.code(409).send({ message: request.t('category_has_questions') });
      if (postgresErrorCode(error) === '23505')
        return reply.code(409).send({ message: request.t('category_name_already_exists') });
      throw error;
    }
