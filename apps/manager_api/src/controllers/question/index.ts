...
function parseMutation(body: unknown): IQuestionMutationInput | null {
  ...
  if (typeof value.category_id !== 'string' || ![1, 2, 3].includes(value.difficulty_level) || !Array.isArray(value.options)) return null;
  const rawTranslations = value.translations as Record<string, { statement?: unknown; explanation?: unknown }> | undefined;
  ...
  const options = value.options.map((option: Record<string, any>) => ({
    ...
    translations: Object.fromEntries(languages.map((language) => [language, { content: text((option.translations as Record<string, { content?: unknown }> | undefined)?.[language]?.content) }])) as IQuestionMutationInput['options'][number]['translations'],
  }));
  if (options.some((option: any) => typeof option.position !== 'number' || typeof option.is_correct !== 'boolean')) return null;
  return { categoryId: value.category_id, difficultyLevel: value.difficulty_level, translations, options } as IQuestionMutationInput;
}

function toEditorQuestion(result: any) {
  const questionTranslations = new Map<string, any>(result.translations.map((translation: any): [string, any] => [translation.language_code, translation]));
  const optionTranslations = new Map<string, any>(result.optionTranslations.map((translation: any): [string, any] => [`${translation.answer_option_id}:${translation.language_code}`, translation]));
  return {
    id: result.question.id, category_id: result.question.category_id, difficulty_level: result.question.difficulty_level,
    status: result.question.status, created_by_user_id: result.question.created_by_user_id,
    created_at: result.question.created_at, updated_at: result.question.updated_at, published_at: result.question.published_at,
    translations: Object.fromEntries(languages.map((language) => {
      const translation = questionTranslations.get(language);
      return [language, translation ? { statement: translation.statement, explanation: translation.explanation } : null];
    })),
    options: result.options.map((option: any) => ({
      id: option.id, position: option.position, is_correct: option.is_correct,
      translations: Object.fromEntries(languages.map((language) => {
        const translation = optionTranslations.get(`${option.id}:${language}`);
        return [language, translation ? { content: translation.content } : null];
      })),
    })),
  };
}
...
export class QuestionController {
  constructor(
    ...
    @inject(QuestionCreatorUseCase) private readonly creator: QuestionCreatorUseCase,
  ...
  create = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!input) return reply.code(400).send({ message: request.t('question_invalid_input') });
    try {
      const question = await this.creator.execute(input, request.authenticatedUser!.id);
  ...
  update = async (request: FastifyRequest, reply: FastifyReply) => {
    const id = (request.params as { id?: string }).id;
    const input = parseMutation(request.body);
    if (!id || !input) return reply.code(400).send({ message: request.t('question_invalid_input') });
    try {
    ...
    try {
      ...
      if (error instanceof QuestionPublishValidationError) {
        return reply.code(409).send({
          ...
          pending: error.pending.map((item) => request.t(`question_publish_pending_${item}`)),
        });
      }
  ...
  remove = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!result) return reply.code(404).send({ message: request.t('question_not_found') });
    return result;
  };

  private mutationError(error: unknown, request: FastifyRequest, reply: FastifyReply) {
