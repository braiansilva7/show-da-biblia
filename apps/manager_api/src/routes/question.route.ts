...
import {
  questionCreatePermissions,
  questionDeletePermissions,
  questionPublishPermissions,
  questionUpdatePermissions,
  questionViewPermissions,
...
import {
  ...
  unpublishQuestionSchema,
  updateQuestionSchema,
  viewQuestionSchema,
} from '@core/schema/question/index.js';
...
export default function questionRoutes(server: FastifyInstance) {
  ...
  server.patch('/questions/:id', {
    ...
    preHandler: [
      (request, reply) =>
        server.authenticateJwt(request, reply, questionUpdatePermissions),
    ],
  });
  server.post('/questions/:id/publish', {
    schema: publishQuestionSchema,
    handler: controller.publish,
    preHandler: [
      (request, reply) =>
        server.authenticateJwt(request, reply, questionPublishPermissions),
    ],
  });
  server.post('/questions/:id/unpublish', {
    schema: unpublishQuestionSchema,
    handler: controller.unpublish,
    preHandler: [
      (request, reply) =>
        server.authenticateJwt(request, reply, questionPublishPermissions),
    ],
  ...
  server.delete('/questions/:id', {
    ...
    preHandler: [
      (request, reply) =>
        server.authenticateJwt(request, reply, questionDeletePermissions),
    ],
  });
}
