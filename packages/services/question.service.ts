...
export class QuestionPublishValidationError extends Error {
  constructor(readonly pending: string[]) {
    super('QUESTION_PUBLISH_INCOMPLETE');
  }
}
...
export class QuestionService {
  ...
  async publish(id: string) {
    const current = await this.repository.findForEdit(id);
    if (!current) return null;
    if (current.question.status === 'PUBLISHED')
      throw new Error('QUESTION_ALREADY_PUBLISHED');
  ...
  async unpublish(id: string) {
    ...
    if (!current) return null;
    if (current.question.status !== 'PUBLISHED')
      throw new Error('QUESTION_NOT_PUBLISHED');
  ...
  private async validate(input: IQuestionMutationInput) {
    ...
    if (
      input.options.length !== answerOptionPositions.length ||
      new Set(input.options.map((option) => option.position)).size !==
        answerOptionPositions.length ||
      !answerOptionPositions.every((position) =>
        input.options.some((option) => option.position === position)
      ) ||
      input.options.filter((option) => option.is_correct).length !== 1
    )
      throw new Error('QUESTION_INVALID_OPTIONS');
    for (const translation of Object.values(input.translations))
      if (
        (translation?.statement && !translation.explanation) ||
        (!translation?.statement && translation?.explanation)
      )
        throw new Error('QUESTION_INVALID_TRANSLATION');
  }

  private async publishPending(
    current: NonNullable<Awaited<ReturnType<QuestionRepository['findForEdit']>>>
  ) {
    const pending: string[] = [];
    if (!(await this.categories.findById(current.question.category_id)))
      pending.push('category');
    if (!isDifficultyLevel(current.question.difficulty_level))
    ...
    if (
      current.options.length !== answerOptionPositions.length ||
      new Set(positions).size !== answerOptionPositions.length ||
      !answerOptionPositions.every((position) => positions.includes(position))
    )
      pending.push('answer_options');
    if (current.options.filter((option) => option.is_correct).length !== 1)
      pending.push('correct_answer');
    const optionIds = new Set(current.options.map((option) => option.id));
    const optionTranslations = current.optionTranslations as Array<{
      answer_option_id: string;
      language_code: QuestionLanguage;
      content: string;
    }>;
    const completeTexts = current.options.every((option) =>
      languages.every((language) => {
        const translation = optionTranslations.find(
          (item) =>
            item.answer_option_id === option.id &&
    ...
    if (
      !completeTexts ||
      optionTranslations.filter((translation) =>
        optionIds.has(translation.answer_option_id)
      ).length !==
        answerOptionPositions.length * languages.length
    )
      pending.push('answer_translations');
    return pending;
  }
}
