...
export const jokerTypes = pgTable('joker_types', {
  ...
  eliminated_wrong_answers: smallint('eliminated_wrong_answers').default(0).notNull(),
  reveals_correct_answer: boolean('reveals_correct_answer').default(false).notNull(),
...
export const jokerUsages = pgTable(
  'joker_usages',
  {
    ...
    joker_type_id: smallint('joker_type_id')
      .references(() => jokerTypes.id)
      .notNull(),
    used_at: timestamp('used_at', { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
  },
...
export const jokerEliminatedOptions = pgTable(
  ...
  (table) => [index('joker_eliminated_options_answer_option_id_idx').on(table.answer_option_id)]
);
