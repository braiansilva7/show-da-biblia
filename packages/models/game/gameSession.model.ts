import { boolean, index, integer, pgTable, smallint, timestamp, uuid, varchar } from 'drizzle-orm/pg-core';
...
export const gameSessions = pgTable(
  ...
  (table) => [index('game_sessions_user_status_started_at_idx').on(table.user_id, table.status, table.started_at)]
);
...
export const sessionQuestions = pgTable(
  'session_questions',
  {
    ...
    selected_answer_option_id: uuid('selected_answer_option_id').references(() => answerOptions.id),
    is_correct: boolean('is_correct'),
    earned_points: integer('earned_points').default(0).notNull(),
    presented_at: timestamp('presented_at', { withTimezone: true, mode: 'string' }),
    answered_at: timestamp('answered_at', { withTimezone: true, mode: 'string' }),
